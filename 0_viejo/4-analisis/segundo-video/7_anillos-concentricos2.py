...
frame_data = []  # Nuevo: Para almacenar datos de cada frame
...
def process_frame(frame, results, show_text=True):
    ...
    global region_counts, total_frames, x_i_accum, frame_data
    ...
    # Nuevo: Guardar datos del frame
    frame_entry = {
        'frame': total_frames,
        'time': total_frames / fps,
        'SI': current_SI,
        'total_stars': int(current_region_counts[:,1].sum()),
        'total_disks': int(current_region_counts[:,0].sum())
    }
    
    # Agregar datos por región
    for i in range(num_regions):
        frame_entry[f'region_{i}_stars'] = int(current_region_counts[i, 1])
        frame_entry[f'region_{i}_disks'] = int(current_region_counts[i, 0])
        frame_entry[f'region_{i}_frac'] = x_i[i] if total_particles[i] > 0 else 0
...
# Nuevo: Guardar datos en CSV
csv_path = os.path.join(output_dir, 'segregation_data.csv')
with open(csv_path, 'w', newline='') as csvfile:
    fieldnames = ['frame', 'time', 'SI', 'total_stars', 'total_disks']
    for i in range(num_regions):
        fieldnames.extend([
            f'region_{i}_stars',
            f'region_{i}_disks',
...
# Nuevo: Generar gráficos básicos
df = pd.read_csv(csv_path)

# Gráfico de evolución del SI
plt.figure(figsize=(10, 5))
plt.plot(df['time'], df['SI'], label='Segregation Index')
plt.xlabel('Tiempo (s)')
plt.ylabel('SI')
plt.title('Evolución del Índice de Segregación')
plt.grid(True)
plt.savefig(os.path.join(output_dir, 'si_evolution.png'))
plt.close()

# Gráfico de distribución radial promedio
avg_frac = [df[f'region_{i}_frac'].mean() for i in range(num_regions)]
plt.figure(figsize=(10, 5))
plt.bar(range(num_regions), avg_frac)
plt.axhline(y=x_o, color='r', linestyle='--', label=f'x_o = {x_o:.3f}')
plt.xlabel('Región')
plt.ylabel('Fracción promedio de estrellas')
plt.title('Distribución Radial Promedio')
plt.legend()
plt.savefig(os.path.join(output_dir, 'radial_distribution.png'))
plt.close()
...
print(f"Datos de segregación guardados en: {csv_path}")
